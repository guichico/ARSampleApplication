...
class ArActivity : AppCompatActivity() {
    ...
    private var objModel: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_ar)

        objModel = intent.getStringExtra("objModel")

        arFragment = scene_form_fragment as ArFragment
    ...
    private fun onUpdate() {
        ...
        if (isTracking && !isModelAdded) {
            if (updateHitTest()) {
                addObject(Uri.parse(String.format("%s%s", objModel, ".sfb")))
                isModelAdded = true;
            }
        }
    }
